...
console.log(cartCount);
  ...
  function getUserDetails (){
    axios.get(url+'user/'+localStorage.getItem("userId"),{ headers: { "Authorization": `Bearer ${localStorage.getItem('Token')}` } })
    .then(res => {
  ...
  useEffect(() => {
    ...
    getCartCount();
    if(isLoggedIn){
    const apic = url+'category/ordercategory/'+ localStorage.getItem('userId')
    ...
    <div>
        ...
        <Grid container spacing={1} style={{ direction: 'row', padding: '15px' }}>
          {products.map((items) => (
            <Grid disabled={items.stock === 0} item xs={6} md={6} lg={3} style={{ marginBottom: 15 }}>
              ...
              <div style={{display:'flex'}}><Typography style={{ top: '-0.75em', fontSize:'10px' }}>RM </Typography>&nbsp;<Typography fontWeight={700}>{items.price}</Typography></div>
      ...
      >
        <DialogTitle>Update your profile</DialogTitle>
            <EditUser handleUserClose={handleProfileEditClose} />
      </Dialog>
